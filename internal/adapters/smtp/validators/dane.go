...
func verifyRecord(r *dns.TLSA, cert *x509.Certificate) error {
	...
	case 0: // Exact match
		if !bytesEqual(data, r.Certificate) { // r.Certificate is the hex string in simple storage, but miekg/dns stores raw bytes in the field usually named Certificate (actually it's a string field "Certificate" in RR, but the struct holds parsed data? Check miekg/dns struct)
